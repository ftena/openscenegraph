...
osg::ref_ptr<osg::Geode> myQuad (std::string textures_path)
{
   ...
   osg::ref_ptr<osg::Image> image1 (osgDB::readImageFile(textures_path+"wood.png"));
   if (image1.get() == 0)
   {
      std::cerr << "wood.png'.\n";
      exit (EXIT_FAILURE);
   ...
   osg::ref_ptr<osg::Image> image2 (osgDB::readImageFile(textures_path+"window04smash.bmp"));
   if (image2.get() == 0)
   {
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()
        ...
        ("textures-path,I", po::value< std::string >(), 
         "path to textures")
        ;
    ...
    if (vm.count("textures-path")) {
        std::cout << "Path to textures is: " 
            << vm["textures-path"].as< std::string >() << std::endl;
        textures_path = vm["textures-path"].as< std::string >() + "/"; 
    } else {
        std::cout << "Path to textures was not set." << std::endl;
        std::cout << desc << std::endl;
        return 1;
    }
	...
	quadPat->addChild(myQuad(textures_path).get());
	...
	osg::ref_ptr<osg::Image> image (osgDB::readImageFile(textures_path+"wood.png"));
	...
	osg::ref_ptr<osg::Node> terrainnode (osgDB::readNodeFile(textures_path+"Terrain2.3ds"));
